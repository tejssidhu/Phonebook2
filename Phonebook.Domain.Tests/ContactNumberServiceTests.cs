...
namespace Phonebook.Tests
{
	...
	public class ContactNumberServiceTests
	{
		...
		public void CreateContactNumberForContactWithTelephoneNumberOfExistingContactNumberOnContactNumberService()
		{
			...
			mockContactRepository.Setup(x => x.Get(It.IsAny<Guid>())).Returns(_contact);
			mockContactNumberRepository.Setup(x => x.GetAll()).Returns(_contactNumbers.AsQueryable());
		...
		public void CreateNewContactNumberForNoneExistentContactOnContactNumberService()
		{
			...
			mockUnitOfWork.Setup(x => x.ContactNumberRepository).Returns(mockContactNumberRepository.Object);
			mockContactNumberRepository.Setup(x => x.GetAll()).Returns(_contactNumbers.AsQueryable());
			mockContactRepository.Setup(x => x.Get(It.IsAny<Guid>())).Returns(() => null);
